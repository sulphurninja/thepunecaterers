...
import {
    ...
    TrendingUp, Award, Shield, Baby, Gift, Sparkles, Coffee, PartyPopper,
...
export default function EventsPage() {
    ...
    const categories = ["all", "celebrations", "ceremonies", "parties", "corporate"];

    const filteredEvents = eventsData.filter(event => {
        const matchesSearch = event.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
            event.description.toLowerCase().includes(searchTerm.toLowerCase());
        const matchesCategory = selectedCategory === "all" ||
            event.keyFeatures.some(feature => feature.toLowerCase().includes(selectedCategory));
...
function Navigation({ isLoaded }: { isLoaded: boolean }) {
    return (
        ...
        >
            <motion.div
                className="bg-white/90 backdrop-blur-xl rounded-2xl px-8 py-3 border border-stone-200/50 shadow-lg"
                whileHover={{ scale: 1.02 }}
                transition={{ type: "spring", stiffness: 300 }}
            >
...
function HeroSection({
    ...
    return (
        <section ref={heroRef} className="relative md:mt-20 min-h-screen flex items-center pt-12 overflow-hidden">
            {/* Enhanced Background */}
            <motion.div
                className="absolute inset-0 bg-gradient-to-br from-purple-50 via-pink-50 to-amber-50"
            ...
            <div className="absolute inset-0 pointer-events-none">
                {[
                    { Icon: PartyPopper, delay: 0, color: "text-purple-600" },
                    { Icon: Heart, delay: 0.2, color: "text-pink-600" },
                    { Icon: Gift, delay: 0.4, color: "text-rose-600" },
                    { Icon: Sparkles, delay: 0.6, color: "text-amber-600" },
                    { Icon: Users, delay: 0.8, color: "text-orange-600" }
                ].map((item, i) => (
                    <motion.div
                        key={i}
            ...
            <div className="container mx-auto px-6 relative z-10">
                <div className="max-w-6xl mx-auto">
                    ...
                    >
                        ...
                        >
                            <motion.span
                                className="inline-flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-purple-100 to-pink-100 text-purple-800 rounded-full text-sm font-medium"
                                initial={{ scale: 0 }}
                                animate={{ scale: 1 }}
                                transition={{ delay: 0.5, type: "spring" }}
                            >
                                <PartyPopper className="w-4 h-4" />
                                <span>Specialized Event Catering Services</span>
                            </motion.span>

                            <h1 className="font-serif text-4xl lg:text-6xl leading-tight text-stone-800">
                                <span className="bg-gradient-to-r from-purple-800 via-pink-800 to-rose-800 bg-clip-text text-transparent">
                                    Exceptional Event Catering
                                </span>
                                <span className="block font-light text-stone-700">Across Pune</span>
                            </h1>
                        </motion.div>
                        ...
                        >
                            From intimate baby showers to grand celebrations, we bring culinary excellence
                            to every special moment. Discover our specialized catering services tailored
                        ...
                        >
                            {[
                                { icon: PartyPopper, number: eventsData.length + "+", label: "Event Types" },
                                ...
                                >
                                    <motion.div
                                        className="w-12 h-12 bg-gradient-to-br from-purple-500 to-pink-600 rounded-xl flex items-center justify-center mx-auto mb-2 group-hover:scale-110 transition-transform duration-300"
                                    >
                                        <stat.icon className="w-6 h-6 text-white" />
                                    </motion.div>
                                    <div className="text-xl font-bold text-purple-800">{stat.number}</div>
            ...
            <div className="container mx-auto px-6">
                ...
                >
                    <div className="text-center mb-8">
                        <h2 className="text-2xl font-serif text-stone-800 mb-4">Find Your Perfect Event</h2>
                        <p className="text-stone-600">Search through our specialized catering services</p>
                    </div>
                    ...
                    <div className="flex flex-col items-center w-full md:flex-row gap-4">
                        ...
                        >
                            ...
                            <Input
                                type="text"
                                placeholder="Search events (e.g., baby shower, wedding...)"
                                value={searchTerm}
                                onChange={(e) => setSearchTerm(e.target.value)}
                                className="pl-12 h-12 rounded-xl border-stone-200 focus:border-purple-400 focus:ring-purple-400/20"
                            />
                        </motion.div>
                        ...
                        >
                            <Select value={selectedCategory} onValueChange={setSelectedCategory}>
                                <SelectTrigger className="h-12 rounded-xl border-stone-200 focus:border-purple-400">
...
// Events Grid
function EventsGrid({ events }: { events: EventData[] }) {
    const ref = useRef(null);
    const isInView = useInView(ref, { once: true, margin: "-100px" });
    ...
    const getEventIcon = (eventName: string) => {
        ...
        return iconMap[eventName] || PartyPopper;
    };
                ...
                <div className="max-w-6xl mx-auto">
                    ...
                    >
                        ...
                        <p className="text-lg text-stone-600 max-w-2xl mx-auto">
                            Discover our specialized catering services designed for different types of celebrations
                        </p>
                    </motion.div>
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                        {events.map((event, i) => {
                            ...
                            const gradient = getEventGradient(i);
                            ...
                            return (
                                ...
                                >
                                    <motion.div
                                        key={event.slug}
                                        className="group relative bg-white rounded-3xl p-8 shadow-lg border border-stone-100 hover:border-purple-200 transition-all duration-500 overflow-hidden"
                                        initial={{ opacity: 0, y: 30 }}
                                        animate={isInView ? { opacity: 1, y: 0 } : {}}
                                    ...
                                    >
                                        ...
                                        {/* Icon */}
                                        <motion.div
                                            className={`w-16 h-16 bg-gradient-to-r ${gradient} rounded-2xl flex items-center justify-center mb-6 shadow-lg group-hover:shadow-xl transition-all duration-300`}
                                        ...
                                        {/* Content */}
                                        <h3 className="font-serif text-2xl text-stone-800 mb-4 group-hover:text-purple-800 transition-colors">
                                            {event.name} Catering
                                        </h3>
                                        ...
                                        <div className="space-y-2 mb-6">
                                            <div className="flex items-center text-sm text-stone-600">
                                                ...
                                                <span>{event.guestRange.min}-{event.guestRange.max} guests</span>
                                            </div>
                                            <div className="flex items-center text-sm text-stone-600">
                                                <TrendingUp className="w-4 h-4 mr-2 text-purple-600" />
                                                <span>From ₹{event.priceRange.starter}/plate</span>
                                            </div>
                                        </div>


                                        <motion.div
                                            className="w-full bg-stone-100 group-hover:bg-purple-600 group-hover:text-white text-stone-800 py-3 rounded-xl font-medium transition-all duration-300 flex items-center justify-center space-x-2 cursor-pointer"
                                            whileHover={{ x: 2 }}
                                            whileTap={{ scale: 0.98 }}
                                        >
                                            <span>Explore {event.name} Catering</span>
                                        ...
                                        {/* Hover Effect */}
                                        <motion.div
                                            className="absolute top-4 right-4 w-8 h-8 bg-white rounded-full shadow-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                                            whileHover={{ scale: 1.1 }}
                                        >
                    ...
                    {events.length === 0 && (
                        ...
                        >
                            <PartyPopper className="w-16 h-16 text-stone-400 mx-auto mb-4" />
                            <h3 className="text-xl font-semibold text-stone-800 mb-2">No events found</h3>
    ...
    const benefits = [
        {
            ...
            description: "Specialized expertise for different types of celebrations with tailored menus and service approaches."
        },
        {
            icon: Users,
            title: "All Guest Capacities",
            description: "From intimate gatherings of 30 to large celebrations of 300+ guests, we handle events of all sizes."
        },
        {
            icon: MapPin,
            title: "Pune-Wide Service",
            description: "Comprehensive coverage across all major Pune locations with local expertise and preferences."
        },
        {
            icon: Shield,
            title: "Quality Guarantee",
            description: "Licensed, insured, and committed to delivering exceptional experiences with every event."
        }
    ];
            ...
            <div className="container mx-auto px-6">
                <div className="max-w-6xl mx-auto">
                    ...
                    >
                        ...
                        <p className="text-lg text-stone-600 max-w-2xl mx-auto">
                            Professional expertise meets heartfelt service for all your special occasions
                        </p>
                    </motion.div>
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {benefits.map((benefit, i) => (
                            <motion.div
                                key={i}
                                className="text-center p-6 rounded-2xl border border-stone-100 hover:border-purple-200 hover:shadow-lg transition-all duration-300 group"
                                initial={{ opacity: 0, y: 30 }}
                                animate={isInView ? { opacity: 1, y: 0 } : {}}
                            ...
                            >
                                <motion.div
                                    className="w-16 h-16 bg-gradient-to-r from-purple-500 to-pink-600 rounded-2xl flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform duration-300"
                                    whileHover={{ rotate: 5 }}
                                >
                                    <benefit.icon className="w-8 h-8 text-white" />
                                </motion.div>
                                <h3 className="font-semibold text-xl text-stone-800 mb-4 group-hover:text-purple-800 transition-colors">
                                    {benefit.title}
                                </h3>
...
function StatsSection() {
    ...
    const stats = [
        {
            ...
            label: "Event Types",
            description: "Specialized catering services",
    ...
    return (
        <section ref={ref} className="py-20 bg-gradient-to-b from-stone-50 to-purple-50">
            <div className="container mx-auto px-6">
                <motion.div
                ...
                >
                    {stats.map((stat, i) => (
                        <motion.div
                            key={i}
                            className="text-center p-6 rounded-2xl bg-white border border-stone-100 hover:border-purple-200 hover:shadow-lg transition-all duration-300 group"
                            initial={{ opacity: 0, y: 20 }}
                            animate={isInView ? { opacity: 1, y: 0 } : {}}
        ...
        <section id="contact" ref={ref} className="py-24 bg-gradient-to-br from-stone-900 via-purple-900 to-pink-900 relative overflow-hidden">
            ...
            <div className="absolute inset-0">
                ...
                {[...Array(3)].map((_, i) => (
                    <motion.div
                        key={i}
                        className="absolute rounded-full bg-gradient-to-r from-purple-400/10 to-pink-500/10 blur-xl"
                        style={{
                            width: `${200 + i * 100}px`,
                            height: `${200 + i * 100}px`,
                            left: `${20 + i * 30}%`,
            ...
            <div className="container mx-auto px-6 relative z-10">
                <div className="max-w-6xl mx-auto">
                    ...
                    >
                        <motion.div
                            className="w-24 h-24 bg-gradient-to-br from-purple-500/20 to-pink-600/20 backdrop-blur-sm rounded-3xl flex items-center justify-center mx-auto mb-8 border border-purple-400/20"
                            initial={{ scale: 0, rotate: -180 }}
                            animate={isInView ? { scale: 1, rotate: 0 } : {}}
                            transition={{ duration: 0.8, delay: 0.2, type: "spring" }}
                            whileHover={{ scale: 1.05, rotate: 5 }}
                        ...
                        >
                            Ready to Plan Your
                            <motion.span
                                className="block bg-gradient-to-r from-purple-400 via-pink-500 to-rose-600 bg-clip-text text-transparent"
                        ...
                        >
                            Join hundreds of families who trusted us with their special moments across Pune.
                        ...
                        >
                            {[
                                ...
                                <motion.a
                                    ...
                                    whileHover={{ y: -5, scale: 1.02 }}
                                >
                                    <div className={`bg-gradient-to-br ${item.gradient} backdrop-blur-sm rounded-2xl p-6 border border-white/10 group-hover:border-purple-400/30 transition-all duration-300`}>
                                        <div className="flex items-start space-x-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-purple-300 font-medium mb-2">{item.content}</p>
                            ...
                            >
                                <div className="text-center p-4 bg-white/10 rounded-2xl backdrop-blur-sm border border-white/10">
                                    <div className="flex items-center justify-center mb-2">
                                        ...
                                        <div className="text-xl font-bold text-purple-400">24hrs</div>
                                    </div>
                                    <div className="text-stone-300 text-sm">Response Time</div>
                                </div>
                                <div className="text-center p-4 bg-white/10 rounded-2xl backdrop-blur-sm border border-white/10">
                                    <div className="flex items-center justify-center mb-2">
                                        <PartyPopper className="w-5 h-5 text-purple-400 mr-2" />
                                        <div className="text-xl font-bold text-purple-400">{eventsData.length}+</div>
                                    </div>
                                    <div className="text-stone-300 text-sm">Event Types</div>
                                </div>
                            </motion.div>
                        </motion.div>
                            ...
                            >
                                <form onSubmit={handleSubmit} className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        >
                                            <Label htmlFor="name" className="text-purple-300 font-medium flex items-center space-x-2">
                                        ...
                                        >
                                            <Label htmlFor="email" className="text-purple-300 font-medium flex items-center space-x-2">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        >
                                            <Label htmlFor="phone" className="text-purple-300 font-medium flex items-center space-x-2">
                                                <Phone className="w-4 h-4" />
                                        ...
                                        >
                                            <Label className="text-purple-300 font-medium flex items-center space-x-2">
                                            ...
                                            <Select onValueChange={(value) => setFormData({ ...formData, eventType: value })}>
                                                ...
                                                <SelectContent className="bg-white border border-stone-200">
                                                    {eventsData.map((event) => (
                                                        <SelectItem key={event.slug} value={event.slug}>{event.name}</SelectItem>
                                                    ))}
                                                </SelectContent>
                                            </Select>
                                        </motion.div>
                                    ...
                                    >
                                        <Label htmlFor="guestCount" className="text-purple-300 font-medium flex items-center space-x-2">
                                            <Users className="w-4 h-4" />
                                    ...
                                    >
                                        <Label htmlFor="message" className="text-purple-300 font-medium flex items-center space-x-2">
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <motion.a
                                                href="tel:+91-8087889252"
                                                className="text-purple-400 hover:text-purple-300 font-semibold px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 transition-all duration-300"
                    ...
                    >
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-2xl mx-auto">
                            {[
                                ...
                                >
                                    <motion.div
                                        className="w-8 h-8 bg-gradient-to-r from-purple-400 to-pink-500 rounded-lg flex items-center justify-center mx-auto mb-2"
                                        whileHover={{ rotate: 5, scale: 1.1 }}
                                    >
                                        <item.icon className="w-4 h-4 text-white" />
                                    </motion.div>
                                    <div className="text-purple-300 font-bold text-sm">{item.value}</div>
                        ...
                        <motion.p
                            ...
                            transition={{ delay: 3 }}
                        >
                            Trusted by families across Pune for exceptional celebrations of all kinds.
